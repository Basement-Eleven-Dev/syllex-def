    ...
    app[httpMethod](expressPath, async (req: Request, res: Response) => {
        try {
            if (role != 'open') {
                ...
                let cognitoGroup: 'students' | 'teachers' | undefined = role == 'teacher' ? 'teachers' : (role == 'student' ? 'students' : undefined)
...
app.listen(PORT, () => {
    ...
    console.table(FUNCTION_INTEGRATIONS.map(l => ({ Method: l.method, Route: l.apiRoute })));

...
export class MainBucket extends Construct {
    bucket: Bucket;
    constructor(scope: Construct, name: string, private lambdaRole: Role) {
        super(scope, name);
    ...
    addNotifications() {
        let lambda = new LambdaConstruct(this, 'create-object-on-db', "src/_triggers/createDatabaseFile.ts", this.lambdaRole).lambda

...
export class RouteConstruct extends NestedStack {
    ...
    private addOptionsMethod(apiResource: IResource) {
        let apiMethod = apiResource.addMethod('OPTIONS', new MockIntegration({
            integrationResponses: [{
                statusCode: '200',
                responseParameters: {
                    'method.response.header.Access-Control-Allow-Headers': "'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,subject-id,Subject-Id'",
    ...
    private addMethod(resource: IResource, functionPath: string, method: string, appRole: AppRole, layers?: { name: string, arn: string }[]) {
    ...
    private createMethods(integrations: FunctionIntegration[]) {
        ...
        integrations.forEach(int => {
            ...
            this.addMethod(resource, int.functionPath, int.method, int.role, int.extensionLayers)
        })
    }

...
const start = async () => {
  ...
  const classes = await db
    .collection("classes")
    .find({ organizationId: new mongo.ObjectId(orgId as string) })
    .toArray();
  const { prefix, count, classId } = await inquirer.prompt([
    { name: "count", type: "number", message: "Quanti studenti?" },
    {
      name: "prefix",
      type: "input",
      message:
        "Prefisso email (le email saranno <prefisso>1@syllex.org, <prefisso>2@syllex.org,...):",
    },
    {
      name: "classId",
      type: "list",
  ...
  await db
    .collection("classes")
    .updateOne({ _id: new mongo.ObjectId(classId as string) }, {

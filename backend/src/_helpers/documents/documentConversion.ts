...
export const getConvertedDocument = async (textContent: string, sourceFileName: string, destinationFileName: string): Promise<Buffer> => {
    ...
    // Pandoc requires double newlines for paragraph breaks. LLMs often use single newlines.
    let pandocFriendlyText = textContent.replace(/\n+/g, '\n\n');

    // Word documents generated by Pandoc often have 0pt paragraph spacing.
    // Force a visual empty line before any Markdown Heading (#, ##, ###) by injecting a non-breaking space.
    pandocFriendlyText = pandocFriendlyText.replace(/\n\n+(?=#+ )/g, '\n\n&nbsp;\n\n');

    // Also force a visual empty line between list items and paragraphs if they seem too squished.
    // Actually, just fixing headings is usually enough to give structure.
    
    await writeFile(sourceFilePath, pandocFriendlyText);

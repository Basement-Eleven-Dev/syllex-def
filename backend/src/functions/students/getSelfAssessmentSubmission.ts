...
export const handler: CustomHandler = async (
  ...
  try {
    ...
    const submissionCollection = db.collection("selfAssessmentSubmissions");

    const aggregationPipeline = [
      { $match: { _id: submissionObjectId, studentId: studentId } },
      {
        $lookup: {
    ...
    if (result.length === 0) {
      return res.status(404).json({ message: "svolgimento non trovato" });
    }

    const submission = result[0];

    if (submission.testDetails && submission.testDetails.questions) {
      submission.testDetails.questions.forEach((q: any) => {
    ...
    return res.status(500).json({
      message: "Errore del server durante il recupero dello svolgimento.",
    });
  }
};

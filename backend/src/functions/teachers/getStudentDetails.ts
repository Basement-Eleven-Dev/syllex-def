...
const getStudentDetails = async (
  ...
  console.log(`Fetching details for student: ${studentId}, class: ${classId}, page: ${page}, limit: ${limit}`);
  ...
  // 2. Resolve subjectId context if classId is provided
  ...
  if (classId && ObjectId.isValid(classId)) {
    ...
    if (classData) {
      ...
      const assignment = await db.collection("teacher_assignments").findOne({ classId: new ObjectId(classId) });
  ...
  const totalAttempts = await db.collection("attempts").countDocuments(attemptMatch);
  ...
  console.log(`Found ${attempts.length} attempts for student (total: ${totalAttempts})`);
  ...
  const allAttemptsInContext = await db.collection("attempts").aggregate([
    { $match: attemptMatch },
    {
      $lookup: {
        from: "tests",
  ...
  const completedAttempts = allAttemptsInContext.filter(a => a.status === 'delivered' || a.status === 'reviewed' || a.status === 'submitted');
  const completedTests = completedAttempts.length;

  const scoredAttempts = allAttemptsInContext.filter(a => a.score != null && a.maxScore > 0);
  let avgScore = 0;
  ...
  const currentSubjectIds = [...new Set(completedAttempts
    .map((a: any) => a.testData?.subjectId || a.subjectId)
    .filter(Boolean)
    .map((id: any) => id.toString())
  )].map(id => new ObjectId(id));

  const subjects = currentSubjectIds.length > 0 
    ? await db.collection("SUBJECTS").find({ _id: { $in: currentSubjectIds } }).toArray()
    : [];
  ...
  for (const s of subjects) {
    if (s?.topics && Array.isArray(s.topics)) {
      for (const t of s.topics) {
        if (t?._id) topicsNameMap.set(t._id.toString(), t.name ?? "Sconosciuto");
      }
    }
  }

  const topicPerformanceMap = new Map<string, { totalScore: number, totalMax: number, name: string }>();
  completedAttempts.forEach((a: any) => {
    (a.questions || []).forEach((q: any) => {
      ...
      const current = topicPerformanceMap.get(topicId) || { totalScore: 0, totalMax: 0, name: topicName };
  ...
  const performanceByTopic = Array.from(topicPerformanceMap.entries()).map(([id, data]) => ({
    topicId: id,
    name: data.name,
    percentage: data.totalMax > 0 ? Math.round((data.totalScore / data.totalMax) * 100) : 0
  })).sort((a, b) => b.percentage - a.percentage);

  const performanceByTest = completedAttempts.slice(0, 10).map((a: any) => ({
    name: a.testData?.name || "Test Sconosciuto",
    score: (a.maxScore || a.testData?.maxScore) > 0 

...
import { faLightbulb, faMagic, faSyncAlt } from '@fortawesome/free-solid-svg-icons';
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-primary bg-opacity-10">
      <div class="card-body p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <button (click)="generateInsight()" class="btn btn-sm btn-outline-primary rounded-pill px-3" [disabled]="loading()">
            <fa-icon [icon]="icons.faSyncAlt" [animation]="loading() ? 'spin' : undefined" class="me-1"></fa-icon>
        ...
        @if (loading()) {
          ...
          <div class="py-4 text-center text-muted">
            <p class="mb-0">Clicca per generare un'analisi assistita dall'IA sul progresso dello studente.</p>
          </div>
        }
      </div>
    </div>
...
export class StudentAiSummaryComponent implements OnInit {
  ...
  generateInsight() {
    ...
    this.studentsService.getStudentInsight(this.studentId).subscribe({
      next: (res) => {
